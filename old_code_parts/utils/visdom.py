...
class VisTwoLinesPlot(VisBase):
    ...
    def draw_data(self):
        assert isinstance(self.raw_data, (list, tuple))
        data_y1 = self.raw_data[0].clone()
        data_y2 = self.raw_data[1].clone()
        data_x = torch.arange(data_y1.shape[0])
        name1 = self.raw_data[2]
        name2 = self.raw_data[3]


        self.visdom.line(data_y1, data_x, opts={'showlegend': True, 'title': self.title}, win=self.title, name=name1, update=None)
        self.visdom.line(data_y2, data_x, opts={'showlegend': True, 'title': self.title}, win=self.title, name=name2, update='append')
...
class Visdom:
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            elif mode == 'twolinesplot':
                self.registered_blocks[title] = VisTwoLinesPlot(self.visdom, show_data, title)
            elif mode == 'Tracking':
