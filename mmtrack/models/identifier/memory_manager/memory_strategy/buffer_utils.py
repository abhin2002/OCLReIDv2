...
class BufferClassTracker(object):
    ...
    def __init__(self, num_class, buffer_size=512, device="cpu"):
        ...
        self.remaining_indexes = set(list(range(buffer_size)))
    ...
    def update_cache(self, buffer_y, new_y=None, ind=None, ):
        ...
        for i, ny, oy in zip(ind, new_y, orig_y):
            ...
            if i in self.remaining_indexes:
                self.remaining_indexes.remove(i)
            
    
    def remove_cache(self, buffer_y, ind):
        # Get labels of memory samples to be replaced
        orig_y = buffer_y[ind]
        for i, oy in zip(ind, orig_y):
            oy_int = oy.item()
            # remove
            if oy_int in self.class_index_cache and i in self.class_index_cache[oy_int]:
                self.class_index_cache[oy_int].remove(i)
...
class BufferTracker(object):
    # For faster label-based sampling (e.g., class balanced sampling), cache class-index via auxiliary dictionary
    # Store {class, set of memory sample indices from class} key-value pairs to speed up label-based sampling
    # class_index_cache: e.g., {<cls_A>: {<ind_1>, <ind_2>}, <cls_B>: {}, <cls_C>: {<ind_3>}, ...}
    # cluster_index_cache: e.g., {<cluster_0>: {<ind_1>, <ind_2>}, <cluster_1>: {}, <cluster_2>: {<ind_3>}, ...}
    # class_set_feature: e.g., {<cls_A>: {<feature_1>, <feature_2>}, <cls_B>: {<feature_1>, <feature_2>}, ...}
    ...
    def __init__(self, num_class, device="cpu"):
        ...
        self.class_set_feature = defaultdict(set)  # sliding window features
        self.cluster_index_cache = defaultdict(set)  # cluster statics

        self.current_index = defaultdict(set)
